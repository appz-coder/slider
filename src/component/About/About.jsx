        ...
        <div>
            ...
            <main role="main" className="container">
                {totalCount ? (
                                    ...
                                    {Presentation.map((pres, i) => {
                                        ...
                                        return (
                                            <tr key={i} className={"control_hover"}>
                                                ...
                                                <td>{pres.title}
                                                    {pres.is_private ? <Image className={"lock"} src={lock} /> : ""}
                                                    <div><small>  {pres.presentation_file[0].mime}</small></div>
                                                </td>
                                                <td>
                                                    <div>
                                                        {pres.is_private ? (
                                                            <button className={"click_btn"} onClick={() => { PrivateChanged(true, pres.secret_key); }}>Private</button>
                                                        ) : (
                                                            <button className={"click_btn"} onClick={() => { PrivateChanged(false, pres.secret_key); }}>Public</button>
                                                        )}
                                                    </div>
                                                </td>
                                                <td>{new Date(pres.createdAt).toLocaleString('en-us', { month: 'short', day: "2-digit" })}</td>
                                                <td className={"d-flex"}>
                                                    ...
                                                    <Button className={"pt-1 pb-1 control_buttons h-25"} variant="outline-dark"
                                                        onClick={() => { ModalShow(pres.presentation_file[0].path, pres.secret_key); }}>
                                                        Share
                                                    </Button>
                                                </td>
                                            </tr>
                    ...
                    <h5 className={"mt-5"}>You do not have any presentations yet</h5>
                )}
                {totalCount > 10 && (
            ...
            <CreatePresentation show={modalShow} onHide={onHide} />
            <SharePresentation show={modalShare} path={path} secretKey={secretKey} onHide={() => setModalShare(false)} />
        </div>
    );
};
